...
class ViewController: UIViewController {
	...
	func configureView() {
		...
		if let classTitle = self.titleClass {
...
//		{
			...
			self.generalChannel?.messages.sendMessage(msg!, completion: nil)
			//{ result in
			textField.text = ""
...
=======
  func textFieldShouldReturn(textField: UITextField) -> Bool {
    let msg = self.generalChannel?
        .messages.createMessageWithBody(textField.text!)
    self.generalChannel?.messages.sendMessage(msg, completion: nil) //{ result in
      textField.text = ""
      textField.resignFirstResponder()
    //}
    return true
